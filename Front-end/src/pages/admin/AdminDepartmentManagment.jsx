...
function AdminDepartmentManagement() {
    ...
    const fetchData = async () => {
      ...
      try {
        const [facultyResponse, departmentResponse] = await Promise.all([
          ...
          fetch("http://localhost:8080/api/departments_with_faculty"),
        ]);

        const facultiesResult = await facultyResponse.json();
        const departmentsResult = await departmentResponse.json();
  ...
  const handleAddEditDepartment = async () => {
    if (!departmentName.trim() || !facultyId) {
  ...
  const filteredDepartments = departments.filter(
    (department) =>
      ...
      department.facultyName.toLowerCase().includes(searchQuery.toLowerCase())
  );
  if (loading) {
    return <Loading />;
  }
  return (
    <>
      <PageTitle title={"Admin Department Management - SmartEduDocs"} />{" "}
                ...
                >
                  ...
                  <Link to={`/departments/courses/${department.id}`}>
                    ...
                    <span className="my-1 block text-gray-600 font-semibold justify-self-start">
                      Faculty of {department.facultyName}
                    </span>
                    <p className="text-sm">
