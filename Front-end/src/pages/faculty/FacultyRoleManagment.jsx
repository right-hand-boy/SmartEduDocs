      ...
      try {
        ...
        const [coursesResponse, departmentsResponse, instructorsResponse] =
          await Promise.all([
            ...
            fetch(
              `http://localhost:8080/api/roles/get/instructors/${user.facultyId}`
            ),
          ]);
        if (
          !coursesResponse.ok ||
    ...
    <>
      ...
      <div className="w-full h-full flex flex-col">
        ...
        <div className="sticky bg-gray-100 top-0 rounded-md z-10 pb-2">
          <div className="mb-4 flex flex-wrap justify-between items-start md:items-center gap-2 rounded-md bg-white p-4 shadow-sm">
            <h1 className="text-lg md:text-2xl whitespace-pre-line font-semibold text-gray-800">
              Role Management
            </h1>
            <button
              onClick={() => setShowAddInstructorModal(true)}
              className="bg-green-600 text-white px-3 py-2 md:px-6 md:py-3 rounded-lg shadow-md hover:bg-green-700 transition duration-200 focus:outline-none focus:ring-2 focus:ring-green-300"
            >
              Add Instructor
            </button>
          </div>
...
export default FacultyRoleManagment;
